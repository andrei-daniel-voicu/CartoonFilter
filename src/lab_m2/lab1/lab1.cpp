...
void Lab1::Init() {
  ...
  // Create a shader program for rendering to texture
  {
    ...
    shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M2, "lab1",
...
void Lab1::Update(float deltaTimeSeconds) {
...

  {
    ...
    int shrink_loc = shader->GetUniformLocation("shrink");
    glUniform1f(shrink_loc, shrink);

    // Note that we only render a single mesh!
    RenderMesh(meshes["bamboo"], shaders["Instances"],
               glm::vec3(-3.3f, 0.5f, 0), glm::vec3(0.1f));
  }
}
...
void Lab1::OnInputUpdate(float deltaTime, int mods) {
  ...
  if (window->KeyHold(GLFW_KEY_9)) {
    shrink -= deltaTime * 0.5f;
    std::cout << "shrinking down\n";
  }
  if (window->KeyHold(GLFW_KEY_0)) {
    shrink += deltaTime * 0.5f;
    std::cout << "shrinking up\n";
  }
  if (shrink < -1) {
    shrink = -1;
  }
