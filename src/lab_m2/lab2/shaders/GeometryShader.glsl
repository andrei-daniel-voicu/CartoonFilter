...
void main()
{
    ...
    if (instance[0] < no_of_instances)
    {
        ...
        float step_gen = 1.0f /  no_of_generated_points;
        float translate_step = instance[0] * 0.25f;

        for(int i = 0; i < no_of_generated_points; i++) {
            vec3 p1 = bezier(i*step_gen);
			vec3 p2 = bezier(i*step_gen + step_gen);

            gl_Position = Projection * View * vec4(translateX(p1, translate_step), 1);
            EmitVertex();
			gl_Position = Projection * View * vec4(translateX(p2, translate_step) , 1);
            EmitVertex();
            gl_Position = Projection * View * vec4(translateX(p1, translate_step + 0.25f) , 1);
            EmitVertex();
            gl_Position = Projection * View * vec4(translateX(p2, translate_step + 0.25f) , 1);
            EmitVertex();
