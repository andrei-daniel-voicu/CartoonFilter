...
void Lab2::Init() {
  ...
  camera->SetPositionAndRotation(glm::vec3(0, 8, 8),
                                 glm::quat(glm::vec3(-40 * TO_RADIANS, 0, 0)));
  camera->Update();
  ...
  // Create a shader program for surface generation
  {
    ...
    shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M2, "lab2",
  ...
  no_of_instances =
      5;  // number of instances (number of curves that contain the surface)
  max_translate = 8.0f;  // for the translation surface, it's the distance
                         // between the first and the last curve
  max_rotate =
      glm::radians(360.0f);  // for the rotation surface, it's the angle between
  ...
  {
    vector<VertexFormat> vertices{VertexFormat(control_p0, glm::vec3(0, 1, 1)),
...
void Lab2::RenderMeshInstanced(Mesh *mesh, Shader *shader,
                               const glm::mat4 &modelMatrix, int instances,
  ...
  glDrawElementsInstanced(mesh->GetDrawMode(),
                          ...
                          GL_UNSIGNED_INT, (void *)0, instances);
}
...
void Lab2::Update(float deltaTimeSeconds) {
  ...
  glUniform3f(glGetUniformLocation(shader->program, "control_p0"), control_p0.x,
              control_p0.y, control_p0.z);
  glUniform3f(glGetUniformLocation(shader->program, "control_p1"), control_p1.x,
              control_p1.y, control_p1.z);
  glUniform3f(glGetUniformLocation(shader->program, "control_p2"), control_p2.x,
              control_p2.y, control_p2.z);
  glUniform3f(glGetUniformLocation(shader->program, "control_p3"), control_p3.x,
              control_p3.y, control_p3.z);
  glUniform1i(glGetUniformLocation(shader->program, "no_of_instances"),
              no_of_instances);
...
void Lab2::OnInputUpdate(float deltaTime, int mods) {
  ...
  auto keyMaps =
      std::vector<std::pair<glm::vec3 &, uint32_t>>{{control_p0, GLFW_KEY_1},
  ...
  for (const auto &k : keyMaps) {
    if (window->KeyHold(k.second)) {
      ...
      std::cout << glm::vec2(control_p0) << glm::vec2(control_p1)
                << glm::vec2(control_p2) << glm::vec2(control_p3) << "\n";
    }
  }
}
...
void Lab2::OnKeyPress(int key, int mods) {
  ...
  if (key == GLFW_KEY_MINUS) {
    if (no_of_instances > 0) no_of_instances--;
  } else if (key == GLFW_KEY_EQUAL) {
    no_of_instances++;
  } else if (key == GLFW_KEY_LEFT_BRACKET) {
    if (no_of_generated_points > 0) no_of_generated_points--;
