...
void Lab6::Init()
{
	...
	camera->SetPositionAndRotation(glm::vec3(0, 2, 3.5),
		glm::quat(glm::vec3(-20 * TO_RADIANS, 0, 0)));
	camera->Update();

	TextureManager::LoadTexture(
		PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES), "ground.jpg");
	...
	for (int i = -gridSize; i < gridSize; i++) {
		for (int j = -gridSize; j < gridSize; j++) {
			...
			L.position =
				glm::vec3(i, rand01, j) * glm::vec3(3, 2, 3) + glm::vec3(0, 0.5, 0);
			L.radius = rand01 + 3;
			lights.push_back(L);
		}
	}
...
void Lab6::Update(float deltaTimeSeconds)
{
	...
	// Lighting pass
	{
		...
		// for each pixel.
		if (cull_type == CullType::FRONT || cull_type == CullType::BACK || cull_type == CullType::ALL)
			glEnable(GL_CULL_FACE);
		if (cull_type == CullType::ALL)
			glCullFace(GL_FRONT_AND_BACK);
		if (cull_type == CullType::FRONT)
			glCullFace(GL_FRONT);
		if (cull_type == CullType::BACK)
			glCullFace(GL_BACK);
...
void Lab6::LoadShader(const std::string& name)
{
	...
	// Create a shader program for particle system
	{
		...
		shader->AddShader(PATH_JOIN(shaderPath, name + ".VS.glsl"),
			GL_VERTEX_SHADER);
		shader->AddShader(PATH_JOIN(shaderPath, name + ".FS.glsl"),
			GL_FRAGMENT_SHADER);
...
void Lab6::OnKeyPress(int key, int mods)
{
	...
	switch (key)
	{
		case GLFW_KEY_Z: cull_type = CullType::NONE;
			break;
		case GLFW_KEY_X: cull_type = CullType::FRONT;
			break;
		case GLFW_KEY_C: cull_type = CullType::BACK;
			break;
		case GLFW_KEY_V: cull_type = CullType::ALL;
			break;
		default:
			break;
	}
